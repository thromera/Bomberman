...
namespace ObjMap
{
  void BlockDes::initialize(void)
  {
    this->texture_ = gdl::Image::load("Textures/caisse.jpg");
  }

  void BlockDes::initialize(float const & x, float const & y, float const & z)
  {
    this->pos_.setPos(x, y, z);
  }

  void BlockDes::update(gdl::GameClock const & gameClock, gdl::Input & input)
  {
    (void) gameClock;
    (void) input;
  }

  void BlockDes::draw(void)
  {
    texture_.bind();
    glPushMatrix();
