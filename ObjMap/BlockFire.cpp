...
namespace ObjMap
{
  void BlockFire::initialize(void)
  {
    this->texture_ = gdl::Image::load("Textures/images-fire.jpg");
  }

  void BlockFire::initialize(float const & x, float const & y, float const & z)
  {
    this->pos_.setPos(x, y, z);
  }

  void BlockFire::update(gdl::GameClock const & gameClock, gdl::Input & input)
  {
    (void) gameClock;
    (void) input;
  }

  void BlockFire::draw(void)
  {
    texture_.bind();
    glPushMatrix();
